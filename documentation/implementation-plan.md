...
### Section 3d: Outline and border as two styles

The frame of a table and the lines between its cells become two independent things with their own style, and the box is renamed to match.

**Table.** `outline` is the frame: `'none'` (the default), `'single'` or `'double'`, or an object with `top`, `right`, `bottom` and `left` each set to one of those, the sides left out being `'none'`. `border` is the lines between the cells, the vertical dividers and the rule rows: `'none'` (the default), `'single'` or `'double'`. The two are independent, so `outline: 'double', border: 'single'` draws a double frame with single dividers, `outline: 'single'` alone a frame without dividers, and `border: 'single'` alone dividers without a frame. `rules: 'all'` still inserts a rule row between every pair of rows; rule rows are drawn in the `border` style, or single when `border` is `'none'`. `corners: 'rounded'` applies when the outline is single. The mask of Section 3c goes: a side of the outline is off by being `'none'`.

The width accounting follows the styles: an inner rule column exists when `border` is not `'none'`, the outer rule column on a side when the outline of that side is not `'none'`. Cell borders of Section 3b keep working as a mask on top: a cell can turn off its sides, whether a side is a divider or a part of the frame, and it can only turn them off.

**Junctions of two styles.** cp437 has the mixed junctions of the DOS box drawing set: `╤ ╧ ╟ ╢ ╪ ╫` and the corners `╒ ╓ ╕ ╖ ╘ ╙ ╛ ╜`. Every junction that can occur has one style per axis, because a horizontal line is either the frame or a rule row and every vertical rule position is either an edge or a divider, so the border helper's table grows from eleven shapes per style to a shape per combination of horizontal and vertical style, and the connectivity map carries a style per bit. Anything with a double line is drawn from cp437; a single outline with rounded corners is drawn as before. The helper verifies once, like Section 1 does for the corners, which page has the glyphs.

**Box.** The `border` option of `box()` is renamed `outline`, without an alias for `border`, which never shipped; the `style` alias from version 3 stays. `documentation/changes.md` says that the option is now called `outline`. Section 1 of this plan described the rename to `border`; this section supersedes it.

**Docs.** The Table section of `commands.md`: the option list with `outline` and `border` as two styles, the width accounting, and every example updated: the ones that today say `border: 'single'` for a framed table say `outline: 'single', border: 'single'`. The Box section and every box example say `outline`. The Markdown section is unaffected (its tables have no lines). `changes.md`: the bullets for the box rename and the table options rewritten to the final names.

**Tests.** Every existing test that builds a framed table or a bordered box updated to the new names. New, in `test/table-outline.js`: the double frame with single dividers as a paper string, on esc-pos with the Epson mapping (cp437 for the whole table) and on star-prnt; single frame with double dividers; a frame without dividers, where the cells have no rule columns between them; dividers without a frame; `outline: { top: 'single', bottom: 'single' }` with `border: 'none'`, which is two horizontal lines and nothing else; rule rows in a table with `border: 'none'` and a double outline connecting with `╟ ╢`; rounded corners ignored with a double outline and applied with a single one next to double dividers; a cell with `border: 'none'` at a corner of a double frame; the randomised geometry check extended with independent random styles for outline and border, checking every glyph's strokes and styles per axis. Every test of the box renamed.
